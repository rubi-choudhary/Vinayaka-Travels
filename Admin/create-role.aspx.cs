...
public partial class Admin_create_role : System.Web.UI.Page
{
            ...
            if (Page.IsValid)
            {
                ...
                if (btnSave.Text == "Update")
                {
                    if (CreateUser.CheckAccess(conSR, pageName, "Edit", Request.Cookies["hi_aid"].Value))
                    {
                        ...
                        ca.Status = "Active";
                        int result = UserRoles.UpdateUserRoles(conSR, ca);
                        if (result > 0)
                        {
                            GetAllRoles();
                            GetRoles();
                ...
                else
                {
                    if (CreateUser.CheckAccess(conSR, pageName, "Add", Request.Cookies["hi_aid"].Value))
                    {
                        ...
                        ca.UpdatedOn = TimeStamps.UTCTime();
                        ca.Status = "Active";
                        int result = UserRoles.InsertUserRoles(conSR, ca);
                        if (result > 0)
                        {
                            txtName.Text = "";
                            GetAllRoles();
    ...
    public void GetAllRoles()
    {
        ...
        {
            strUserRoless = "";
            List<UserRoles> cas = UserRoles.GetAllUserRoles(conSR).OrderByDescending(s => Convert.ToDateTime(s.UpdatedOn)).ToList();
            int i = 0;
...
<td>" + ca.UpdatedOn.ToString("dd-MMM-yyyy") + @"</td>  
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conSR = new SqlConnection(ConfigurationManager.ConnectionStrings["conSR"].ConnectionString);
            if (CreateUser.CheckAccess(conSR, "create-role.aspx", "Delete", HttpContext.Current.Request.Cookies["hi_aid"].Value))
            {
                UserRoles cat = new UserRoles();
                cat.Id = Convert.ToInt32(id);
                cat.UpdatedOn = TimeStamps.UTCTime();
                int exec = UserRoles.DeleteUserRoles(conSR, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
