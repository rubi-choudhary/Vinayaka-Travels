...
public partial class Admin_view_admin_users : System.Web.UI.Page
{
    ...
    public void GetAllUsers()
    {
        ...
        {
            strUsers = "";
            List<CreateUser> instrs = CreateUser.GetAllUser(conSR);
            int i = 0;
...
<div class='form-check form-switch markBlocked' id='chk_" + cat.Id + @"'  data-id='" + cat.Id + @"'>
  ...
  <td>" + chk + @"</td> 
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conSR = new SqlConnection(ConfigurationManager.ConnectionStrings["conSR"].ConnectionString);
            if (CreateUser.CheckAccess(conSR, "view-admin-users.aspx", "Delete", HttpContext.Current.Request.Cookies["hi_aid"].Value))
            {
                ...
                cat.AddedBy = HttpContext.Current.Request.Cookies["hi_aid"].Value;
                int exec = CreateUser.DeleteUser(conSR, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
    ...
    public static string BlockUsers(string id, string ftr)
    {
        ...
        try
        {
            SqlConnection conGV = new SqlConnection(ConfigurationManager.ConnectionStrings["conSR"].ConnectionString);
            if (CreateUser.CheckAccess(conGV, "view-admin-users.aspx", "Edit", HttpContext.Current.Request.Cookies["hi_aid"].Value))
            {
                int exec = 0;
                CreateUser pro = new CreateUser();
                if (ftr == "Yes")
                {
                    exec = CreateUser.BlockUser(conGV, id, HttpContext.Current.Request.Cookies["hi_aid"].Value);
                    vali = "Blocked";
                }
                else
                {
                    exec = CreateUser.UnBlockUser(conGV, id, HttpContext.Current.Request.Cookies["hi_aid"].Value);
                    vali = "UnBlocked";
                }
